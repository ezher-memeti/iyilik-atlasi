    ...
    <div ref={containerRef} className="mx-auto w-full max-w-3xl">
      <div className="flex gap-2.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:pb-0">
        {safeStats.map((stat, index) => (
          <article
            key={stat.label}
            className={`group min-w-[31%] flex-1 rounded-xl border border-divider-softLight bg-surface-pageLight/80 px-3 py-3 text-center shadow-sm backdrop-blur-sm transition duration-300 hover:-translate-y-0.5 hover:shadow-soft sm:min-w-0 sm:px-4 sm:py-4 ${hasStarted
