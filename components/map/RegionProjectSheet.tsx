...
export function RegionProjectSheet({
  title,
  projects,
  onSelectProject,
  selectedProjectIds = [],
      ...
      <div className="border-b border-divider-softLight px-4 py-3">
        ...
        {ctaLabel && onCtaClick ? (
          <button
            type="button"
            onClick={onCtaClick}
            className="mt-2 inline-flex h-9 items-center justify-center rounded-full border border-emerald-200 bg-emerald-50 px-3 text-xs font-semibold text-emerald-800 transition hover:bg-emerald-100"
          >
            {ctaLabel}
          </button>
        ) : null}
